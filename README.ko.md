...
요금제 배지, 주간 잔여 사용 한도 %, 초기화까지 남은 시간을 표시합니다. 데모에는 주간 한도만 제공하며 5시간 한도를 추가하지 않습니다. 데모 데이터는 명시적으로 표시한 샘플입니다. `--live` 모드에서는 Codex가 로컬 세션 기록(`CODEX_HOME/sessions`)에 직접 남긴 마지막 사용량 값을 읽어 표시하며, 서비스에 사용량을 새로 요청하지 않습니다. '남음'은 토큰 잔액이 아니라 해당 시간 구간의 사용 한도 비율입니다. 실제 응답의 windowDurationMins로 구간 이름을 정하며, 누락값은 미확인, 초기화 시각이 지난 값은 새로고침 필요로 표시합니다.

공식 응답 형식의 파서, UI, 조회 전후 계정 확인 로직을 구현하고 가짜 응답으로 검증했습니다. 실시간 조회 연결은 여전히 남아 있습니다. 대신 참고 프로젝트와 같은 방식으로 Codex 세션 기록의 사용량 줄만 읽고, 앱이 기록한 저장·전환 이력으로 계정에 귀속합니다. 앱 밖에서 로그인이 바뀐 구간의 값은 귀속하지 않으며, 앱에 저장하기 전의 값은 처음 저장한 로그인 계정의 것으로 간주합니다. [조회 연결 준비](docs/read-only-connection.md)와 [벤치마크 결과](docs/benchmark.md)를 참고하세요.
...
기본 `~/.codex/auth.json` 또는 실행 환경의 `CODEX_HOME/auth.json`을 사용합니다. 계정 사본은 `~/Library/Application Support/CodexSwitch`에 저장합니다. 폴더 0700, 파일 0600 권한의 **평문**이므로 macOS 로그인 사용자와 관리자에게는 읽힐 수 있습니다. 네트워크 요청과 토큰 로그는 없습니다. 세션 기록은 사용량 줄만 추출하고 대화 내용은 읽거나 저장하지 않습니다.
...
먼저 기본 데모로 표시·언어·테마·전환 실패 시나리오를 확인하세요. 실제 계정에는 Codex가 로컬에 남긴 마지막 사용량 값이 있을 때만 표시되며, 패널을 다시 열어서가 아니라 Codex를 사용해야 갱신됩니다.
