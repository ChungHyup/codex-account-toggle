...
public final class SwitchCoordinator {
    ...
    public func switchTo(_ profile: Profile, progress: (String) -> Void = { _ in }) async throws {
        guard !isSwitching else { throw SwitchError("이미 계정 전환을 진행 중입니다.") }
        isSwitching = true
        ...
        guard !FileManager.default.fileExists(atPath: store.backup.path) else {
            throw SwitchError("먼저 이전 로그인을 복구하세요.")
        }
        let target = try store.auth(profile)
        if try Identity(data: store.read(store.active)).key == profile.id { return }
        progress("앱이 정상 종료되기를 기다리는 중…")
        try await lifecycle.quit()
        guard try !lifecycle.isRunning() else { throw SwitchError("앱이 아직 실행 중입니다. 로그인은 변경하지 않았습니다.") }
        do {
            try store.replace(with: target)
            progress("로그인 교체 완료 · 앱 실행 중…")
            try await lifecycle.launch()
            try store.commit()
        } catch {
            if FileManager.default.fileExists(atPath: store.backup.path) {
                guard try !lifecycle.isRunning() else {
                    throw SwitchError("전환을 완료하지 못했습니다. 앱을 종료한 뒤 이전 로그인 복구를 실행하세요.")
                }
                do { try store.rollback() }
                catch { throw SwitchError("자동 복구에 실패했습니다. 복구 파일은 보존되어 있습니다.") }
                try? await lifecycle.launch()
                throw SwitchError("전환 실패로 이전 로그인을 복구했습니다.")
            }
            throw error
        }
    }
...
public enum DemoScenario: String, CaseIterable, Identifiable {
    ...
    public var title: String {
        switch self {
        case .success: return "정상 전환"
        case .quitRefused: return "종료 거부"
        case .launchFailed: return "재실행 실패 · 복구"
        }
    }
}
...
public final class DemoLifecycle: AppLifecycle {
    ...
    public func quit() async throws {
        quitCount += 1
        if scenario == .quitRefused { throw SwitchError("데모: 종료 요청이 거부되어 전환을 취소했습니다.") }
        running = false
...
public enum DemoWorkspace {
    ...
    public static func make(in directory: URL = FileManager.default.temporaryDirectory) throws -> Store {
        ...
        try store.save(data: first, name: "개인 계정")
        try store.save(data: credentials(user: "work", email: "work@example.test"), name: "업무 계정")
        try store.save(data: credentials(user: "side", email: "side@example.test"), name: "사이드 프로젝트")
