...
public struct Identity: Equatable {
    ...
    public init(data: Data) throws {
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw SwitchError("지원하는 ChatGPT 로그인 파일이 아닙니다.")
        }
        ...
        email = claims["email"] as? String ?? "ChatGPT 계정"
        // Include the user as well as the workspace to avoid merging organization members.
        let stable = account + ":" + (claims["sub"] as? String ?? email)
...
public final class Store {
        ...
        if FileManager.default.fileExists(atPath: config.path) {
            ...
            if let match = regex.firstMatch(in: text, range: NSRange(text.startIndex..., in: text)),
               let range = Range(match.range(at: 1), in: text), text[range] != "file" {
                throw SwitchError("현재 인증 저장 방식은 지원하지 않습니다. 파일 기반 로그인만 전환할 수 있습니다.")
            }
        }
    ...
    public func rename(_ profile: Profile, to name: String) throws {
        ...
        guard !cleaned.isEmpty, cleaned.count <= 80 else { throw SwitchError("계정 이름은 1~80자로 입력하세요.") }
        var list = try profiles()
        guard let position = list.firstIndex(where: { $0.id == profile.id }) else { throw SwitchError("저장된 계정을 찾을 수 없습니다.") }
        list[position].name = cleaned
        try write(JSONEncoder().encode(list), to: index)
    }
    public func read(_ url: URL) throws -> Data {
        let attrs = try FileManager.default.attributesOfItem(atPath: url.path)
        guard attrs[.type] as? FileAttributeType == .typeRegular else { throw SwitchError("일반 파일만 사용할 수 있습니다.") }
        return try Data(contentsOf: url)
    }
    public func auth(_ profile: Profile) throws -> Data {
        guard profile.id.count == 64, profile.id.allSatisfy({ $0.isHexDigit }) else { throw SwitchError("저장된 계정 식별자가 올바르지 않습니다.") }
        let data = try read(root.appendingPathComponent(profile.id + ".auth.json"))
        guard try Identity(data: data).key == profile.id else { throw SwitchError("저장된 계정과 로그인 파일이 일치하지 않습니다.") }
        return data
    }
    ...
    public func write(_ data: Data, to url: URL) throws {
        ...
        guard fd >= 0 else { throw SwitchError("보안 임시 파일을 만들 수 없습니다.") }
        defer { Darwin.close(fd); try? FileManager.default.removeItem(at: temp) }
        try data.withUnsafeBytes { buffer in
            var offset = 0
            while offset < buffer.count {
                let count = Darwin.write(fd, buffer.baseAddress!.advanced(by: offset), buffer.count - offset)
                guard count > 0 else { throw SwitchError("로그인 파일 저장에 실패했습니다.") }
                offset += count
            }
        }
        guard fsync(fd) == 0, Darwin.rename(temp.path, url.path) == 0 else { throw SwitchError("로그인 파일 교체에 실패했습니다.") }
    }
    public func replace(with data: Data) throws {
        ...
        try prepare()
        guard !FileManager.default.fileExists(atPath: backup.path) else { throw SwitchError("이전 전환의 복구 파일이 있습니다. 먼저 이전 로그인 복구를 실행하세요.") }
        ...
        guard try read(active) == data else { throw SwitchError("교체된 로그인 파일 검증에 실패했습니다.") }
    }
